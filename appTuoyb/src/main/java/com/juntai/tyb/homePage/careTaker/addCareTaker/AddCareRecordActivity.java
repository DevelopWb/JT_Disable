...
import com.juntai.tyb.base.selectPics.BaseSelectPicActivity;
import com.juntai.tyb.base.selectPics.SelectPhotosFragment;
import com.juntai.tyb.bean.careTaker.CareRecoredSavedBean;
import com.juntai.tyb.bean.careTaker.CareTakerInfoBean;
import com.juntai.tyb.bean.careTaker.CareTakerPicBean;
...
//                        servicePeopleAdapter.setNewData(allServices);
    ...
    public void onClick(View v) {
        int id = v.getId();
        if (id == R.id.add_location_addr_tv) {//选择地点
            if (isInnerAccount()) {
        ...
        } else if (id == R.id.service_controler_ll) {//开始结束服务
        ...
        } else if (id == R.id.filter_iv) {//过滤服务人员
        ...
        } else if (id == R.id.service_people_area_tv) {
            mFilterIv.performClick();
        } else if (id == R.id.save_record_tv) {//保存开始时间到本地
        ...
        } else if (id == R.id.commit_record_tv) {
        ...
        } else if (id == R.id.service_start_time_value_tv) {//首先看是否选中服务人员，因为需要根据人员信息获取对应的所有托养记录里面的服务时间段   解决网段重叠的问题
        ...
        } else if (id == R.id.service_end_time_value_tv) {//结束服务的时间
            if (isInnerAccount()) {
                PickerManager.getInstance().showTimePickerView(mContext, PickerManager.getInstance().getTimeType(
                        "minute"), "", new PickerManager.OnTimePickerTimeSelectedListener() {
                    @Override
        ...
        } else if (id == R.id.service_type_value_tv) {//选择定位类型
