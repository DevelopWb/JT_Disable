package com.juntai.disabled.federation.home_page.weather;
...
import com.juntai.disabled.federation.bean.weather.WeatherDaysBean;
import com.juntai.disabled.federation.bean.weather.WeatherEveryDayBean;
import com.juntai.disabled.federation.bean.weather.WeatherHoursBean;
...
public class WeatherActivity extends BaseMvpActivity<HomePagePresent> implements HomePageContract.IHomePageView, View.OnClickListener {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        if (requestCode == SelectLocation.SELECT_LOCATION && resultCode == SelectLocation.RESULT_OK) {
...
//        Intent intent = getIntent();
        if (LocateAndUpload.bdLocation != null){
            province = LocateAndUpload.bdLocation.getProvince();
            city = LocateAndUpload.bdLocation.getCity();
            area = LocateAndUpload.bdLocation.getDistrict();
            requestWeather(String.valueOf(LocateAndUpload.bdLocation.getLongitude()), String.valueOf(LocateAndUpload.bdLocation.getLatitude()));
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.weather_place:
                startActivityForResult(new Intent(mContext, SelectLocation.class).putExtra("city", city), SelectLocation.SELECT_LOCATION);
                break;
            default:
                break;
            case R.id.back_btn:
